...
def get_psv_regions(record, genome):
    yield Interval(genome.chrom_id(record.chrom), record.start, record.start + len(record.ref))
    for pos2 in record.INFO['pos2']:
        pos2 = pos2.split(':')
        chrom = pos2[0]
        start = int(pos2[1]) - 1
        allele_ix = int(pos2[3]) if len(pos2) >= 4 else 1
        yield Interval(genome.chrom_id(chrom), start, start + len(record.alleles[allele_ix]))


def load_psvs(psvs_filename, genome):
    if psvs_filename is None:
        return None

    psv_tree = itree.MultiChromTree()
    with pysam.VariantFile(psvs_filename) as inp:
        for record in inp:
            for interval in get_psv_regions(recod, genome):
                psv_tree.add(interval, None)
    return psv_tree
...
def generate_chrom(genome, chrom_id, psv_tree, mut_rates, hetero_rate, out_vcf, rs):
    ...
    with tqdm(total=chrom_len) as pbar:
        ...
        while True:
            ...
            if psv_tree is not None:
                region = Interval(chrom_id, i - 1, i + len(ref) + 1)
                if psv_tree.n_overlaps(region) > 0:
                    i += 1
                    pbar.update()
                    continue
...
def main():
    ...
    parser.add_argument('-l', '--indel-len', type=int, default=4,
        help='Generate insertions and deletions of size between 1 and INT [default: %(default)s].')
    parser.add_argument('-o', '--output', required=True,
        help='Output VCF file.')

    parser.add_argument('-p', '--psvs', required=False,
        help='Optional: PSVs VCF file. Used for excluding generated variants that overlap PSVs.')
    ...
    psv_tree = load_psvs(args.psvs, genome)
    ...
    with pysam.VariantFile(args.output, 'wz' if args.output.endswith('.gz') else 'w', header=header) as out_vcf:
        for chrom_id, chrom in enumerate(genome.chrom_names):
            if chroms is None or chrom in chroms:
                ...
                generate_chrom(genome, chrom_id, psv_tree, args.mutation_rate, args.hetero_rate, out_vcf, rs)
