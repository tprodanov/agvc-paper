...
events <- read.csv('cmp_WGS_WES.events.csv.gz', sep = '\t') |>
    inner_join(select(cmp, pos, rare, ref_cn), join_by(pos)) |>
    mutate(
...
x_lab <- ggplot() + 
  annotate(geom = 'text', x = 0, y = 0, label = 'WES allele dosage', family = 'Carlito') +
  coord_cartesian(clip = 'off') +
  theme_void()
y_lab <- ggplot() + 
  annotate(geom = 'text', x = 0, y = 0, label = 'WGS allele dosage', family = 'Carlito', angle = 90) +
  coord_cartesian(clip = 'off') +
  theme_void()
